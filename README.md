[index.html](https://github.com/user-attachments/files/26162894/index.html)
<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .empty-ico { font-size:36px; display:block; margin-bottom:12px; }
...
<script>
// =====================================================================
//   LISTE DES VIDEOS — ces sections sont mises a jour automatiquement
//   par GitHub Actions chaque nuit. Tu peux aussi modifier manuellement.
// =====================================================================

const VIDEOS = {

  // A LA UNE — modifie toi-meme, non mis a jour automatiquement
  alune: [
  ...
  // Mis a jour automatiquement par GitHub Actions
  documentaires: [
    { id:"PCMrUGqH6fQ", titre:"La face cachée des océans", date:"Fév 2025" },
    { id:"7Xfk9CN6S4E", titre:"L'univers en 1 heure", date:"Jan 2025" },
  ],
...
function safeSrc(id, quality) {
  // On utilise un placeholder gris si l'image echoue — jamais de fallback vers index.html
...
function renderCat(cat) {
  ...
  if (!list.length) {
    ...
    document.getElementById('grid').innerHTML = `<div class="empty-box"><span class="empty-ico">🎬</span>Aucune vidéo dans cette catégorie.<br><small>Ouvre <b>index.html</b> et ajoute des vidéos dans la section <b>${cat}</b>.</small></div>`;
    return;
  }
  ...
  document.getElementById('hero-zone').innerHTML = `
    <div class="hero" onclick="openPlayer('${f.id}','${esc(f.titre)}')">
      <img id="hero-img"
           src="${safeSrc(f.id,'maxresdefault')}"
           alt="${esc(f.titre)}"
           onerror="if(!this.dataset.tried){this.dataset.tried=1;this.src='${safeSrc(f.id,'hqdefault')}';}else{this.style.opacity='0';}">
  ...
  document.getElementById('grid').innerHTML = list.slice(1).map(v => `
    <div class="vcard" onclick="openPlayer('${v.id}','${esc(v.titre)}')">
      <div class="thumb-box">
        <img class="vthumb"
             src="${safeSrc(v.id,'mqdefault')}"
             alt="${esc(v.titre)}"
             onerror="if(!this.dataset.tried){this.dataset.tried=1;this.src='${safeSrc(v.id,'hqdefault')}';}else{this.style.opacity='0';}">
